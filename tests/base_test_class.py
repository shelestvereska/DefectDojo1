...
def on_exception_html_source_logger(func):
    def wrapper(self, *args, **kwargs):
        try:
            ...
            logger.info("exception occured at url:", self.driver.current_url)
...
class BaseTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not dd_driver:
            ...
            logger.info("launching browser for: ", cls.__name__)
    ...
    def set_block_execution(self, block_execution=True):
        ...
        logger.info("setting block execution to: ", str(block_execution))
        driver = self.driver
    ...
    def tearDownDriver(cls):
        logger.info("tearDownDriver: ", cls.__name__)
        global dd_driver
        if dd_driver:
            if (
